    ...
    <Link href={`/exercise/${exercise.id}`} className="surface flex min-h-[88px] items-center gap-3 rounded-2xl p-3 transition active:scale-[.99]">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          {complete && (
            <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[color:var(--accent)] text-white">
              <Check size={13} />
            </span>
          )}
        </div>
        <p className="mt-1 text-sm font-medium">{exercise.target}</p>
        <p className="mt-1.5 inline-flex items-center gap-1 text-[11px] font-semibold uppercase tracking-[.1em]" style={{ color: meta.color }}>
